...
def download_pokedex_data_to_json():
    ...
    for i in range(0, len(pokemon_list)):
        ...
        info_log("Loaded pekemon #" + str(id) + ", name: " + name)


    json_object = json.dumps(json_object, indent=4)
    with open(POKEDEX_DATA_PATH, "w") as outfile:
        outfile.write(json_object)
    info_log("Wrote pokedex data to " + POKEDEX_DATA_PATH + " sccessfully!")
...
def download_moves_data_to_json():
    ...
    with open(MOVES_DATA_PATH, "w") as outfile:
        outfile.write(json_object)
        info_log("Wrote moves data to " + MOVES_DATA_PATH + " sccessfully!")
...
def download_pokemon_sprites():
    ...
    def download_sprite(url, file_name):
        ...
        if res.status_code == 200:
            ...
            info_log('image sucessfully Downloaded: ' + file_name)
        else:
            info_log('image Couldn\'t be retrieved')
    ...
    for i in range(0, len(pokemon_list)):
        ...
        if front_sprite_url != None: download_sprite(front_sprite_url, POKEMON_SPRITES_FOLDER_PATH + str(id) + '_front.png')
        if back_sprite_url != None: download_sprite(back_sprite_url, POKEMON_SPRITES_FOLDER_PATH + str(id) + '_back.png')
